...
InlineLexer.prototype.output = function(src) {
  ...
  while (src) {
    ...
    // hashtag
    if (!this.inLink && this.options.hashtags && (cap = this.rules.hashtag.exec(src))) {
      src = src.substring(cap[0].length);
      out += this.renderer.hashtag(cap[1], cap[2])
      continue;
    }
...
Renderer.prototype.hashtag = function(preceding, tag) {
  var href = this.urltransform(tag)
  if (href === false)
    return (preceding||'')+'<a class="bad">'+escape(tag)+'</a>'
  return (preceding||'')+'<a href="'+escape(href)+'">'+escape(tag)+'</a>'
}
